...
import {
  ...
  getAllListCO,
  getAllListNSX,
  locMauSac, // Thêm hàm tìm kiếm theo màu sắc
...
const handleColorChange = async (color) => {
  // Tạo một bản sao mới của mảng màu sắc đã chọn
  const updatedSelectedColors = [...selectedColors];

  // Kiểm tra xem màu sắc đã được chọn trước đó chưa
  if (updatedSelectedColors.includes(color)) {
    // Nếu đã chọn trước đó, loại bỏ màu sắc khỏi danh sách đã chọn
    updatedSelectedColors.splice(updatedSelectedColors.indexOf(color), 1);
  } else {
    // Nếu chưa chọn trước đó, thêm màu sắc vào danh sách đã chọn
    updatedSelectedColors.push(color);
  }

  // Cập nhật mảng màu sắc đã chọn
  setSelectedColors(updatedSelectedColors);

  // Gọi hàm `locMauSac` để lọc sản phẩm dựa trên màu sắc đã chọn
  const selectedColorsStr = updatedSelectedColors.join(',');

  try {
    const filteredData = await locMauSac(selectedColorsStr, 0);
    setFilteredData(filteredData.data); // Cập nhật danh sách sản phẩm đã lọc
  } catch (error) {
    // Xử lý lỗi nếu có
    console.error('Lỗi khi lọc sản phẩm theo màu sắc:', error);
  }
};
