...
// import { getAllCTSPWeb, getAllListCL, getAllListKC, getAllListMS, getAllListCO, getAllListNSX, locMauSac } from 'services/SanPhamService';
...
//   // Gọi API "locMauSac" để lọc sản phẩm bằng màu sắc
//   locMauSac(selectedColorIds, page).then((res) => {
//     if (res && res.data) {
//       // Lọc theo màu sắc và cập nhật filteredData
//       const colorFilteredProducts = priceFilteredProducts.filter((product) => {
//         return res.data.some((item) => item.mauSac.ten === product.mauSac.ten);
//       });
//       setFilteredData(colorFilteredProducts);
//     }
//   });
...
//   };
//   useEffect(() => {
//     // Gọi API "locMauSac" khi danh sách các màu sắc được chọn thay đổi
//     const selectedColorIds = selectedColors.map((color) => color.id);
//     const page = 0; // Trang bạn muốn lấy (nếu cần)
//     locMauSac(selectedColorIds, page).then((res) => {
//       if (res) {
...
// import React, { useState, useEffect } from 'react';
// import Card from 'react-bootstrap/Card';
// import { getAllCTSPWeb, getAllListCL, getAllListKC, getAllListMS, getAllListCO, getAllListNSX, locCTSP } from 'services/SanPhamService';
// import '../../scss/ChiTietSanPham.scss';
// import ReactPaginate from 'react-paginate';
...
//         setPriceRange([MIN, maxProductPrice]);
//         setDisplayedPriceRange([MIN, maxProductPrice]);
//       }
//     });
//   }, []);
...
//     const mauSac = selectedColors.join(',');
//     const kichCo = selectedSizes.join();
//     const chatLieu = selectedMaterials.join();
//     const coAo = selectedCollars.join();
//     const nhaSanXuat = selectedManufacturers.join();

//     locCTSP(mauSac, kichCo, chatLieu, coAo, nhaSanXuat, 0)
//       .then((res) => {
//         if (res) {
...
//   const handlePriceRangeChange = (newValues) => {
//     setPriceRange(newValues);
//     setDisplayedPriceRange(newValues);
//     filterProducts();
//   };
...
import React, { useState, useEffect } from 'react';
import Card from 'react-bootstrap/Card';
import { getAllCTSPWeb, getAllListCL, getAllListKC, getAllListMS, getAllListCO, getAllListNSX, locCTSP } from 'services/SanPhamService'; // Thêm import
import ReactPaginate from 'react-paginate';
import Accordion from 'react-bootstrap/Accordion';
import Form from 'react-bootstrap/Form';
import Slider from 'react-slider';
...
function ContentSanPham() {
  ...
  const filterProducts = () => {
    locCTSP(
      {
        mauSac: selectedColors.join(),
        kichCo: selectedSizes.join(),
        chatLieu: selectedMaterials.join(),
        coAo: selectedCollars.join(),
        nhaSanXuat: selectedManufacturers.join(),
        minGiaBan: priceRange[0],
        maxGiaBan: priceRange[1]
      },
      0
    )
    .then((res) => {
  ...
  const handlePageClick = (event) => {
    getAllCTSPWeb(event.selected).then((res) => {
      if (res) {
        setData(res.data.content);
        setTotalPages(res.data.totalPages);
        filterProducts();
...
//     setPriceRange(newValues);
//     setDisplayedPriceRange(newValues); // Cập nhật displayedPriceRange
//     filterProducts(); // Gọi hàm filterProducts khi khoảng giá thay đổi
//   };
                    ...
                    <Form.Group key={index} className="color-filter" controlId={`colorCheckbox${index}`}>
                      <label className=" round-checkbox-container" style={{display: 'contents', marginLeft: '5px'}}>
                        <input
                          ...
                          checked={selectedColors.includes(color.ten)}
                        />
                        <Form className="color-code" style={{ backgroundColor: color.ten, marginLeft: '6px' }}/>
                        <span className="round-checkmark"></span>
                      </label>
